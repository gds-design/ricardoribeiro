...
export default function CaravanaIsraelPage() {
  ...
  const benefits = [
    ...
    { title: "Mentoria Espiritual", desc: "Acompanhamento pastoral diário e ensinamentos profundos in loco.", icon: <UserCheck className="w-5 h-5 text-amber-500" /> },
  ...
  return (
    <div className="font-sans antialiased bg-slate-50 text-slate-900 select-none">
      ...
      {/* 1. Hero Section */}
      <section className="relative min-h-[75vh] bg-slate-950 flex flex-col justify-center overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-30" 
          style={{ backgroundImage: "url('/jerusalem-modern.png')" }} 
        />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(15,23,42,0.7),#020617)] z-0" />
        
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 z-10 w-full text-center space-y-6">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-white rounded-full text-[10px] font-bold uppercase tracking-wider mx-auto">
            <Plane className="w-3.5 h-3.5" />
            <span>Viagem de Destino</span>
          </span>

          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-extrabold text-white tracking-tight max-w-4xl mx-auto leading-tight">
            Caravana Profética para Israel
          </h1>

          <p className="text-sm sm:text-base text-slate-350 max-w-xl mx-auto leading-relaxed">
            Não é apenas uma viagem. É uma experiência espiritual.
          </p>
          ...
          <div className="pt-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-6 py-3.5 bg-amber-500 hover:bg-amber-455 text-slate-950 rounded-xl text-xs font-black uppercase tracking-wider shadow-lg border-0 cursor-pointer inline-flex items-center gap-2"
            >
            ...
            {timeline.map((step, idx) => (
              <div key={idx} className="relative">
                ...
                <span className={`absolute -left-[31px] top-1.5 w-4 h-4 rounded-full border-2 bg-white flex items-center justify-center ${
                  idx === 0 ? "border-amber-500 ring-4 ring-amber-500/20" : "border-slate-300"
                }`} />
                <div className="space-y-1">
                  <h4 className="text-xs font-black text-slate-950 uppercase tracking-wider flex items-center gap-1.5">
                    <span>{step.title}</span>
                    {idx === 0 && <span className="text-[8px] bg-amber-500 text-slate-950 font-black px-1.5 py-0.5 rounded">Início</span>}
                  </h4>
        ...
        <div className="relative max-w-xl mx-auto px-4 z-10 space-y-6">
          ...
          <div className="pt-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-8 py-4 bg-amber-500 hover:bg-amber-455 text-slate-950 rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 shadow-2xl transition-colors border-0"
            >
